        ...
        <div class="row">
            ...
            <div class="col-12 col-md-3 form-group mb-0">
                <select name="type" class="form-control form-select select2" data-placeholder="Select Type">
                    ...
                    <option {{request('type') == $type->id ? 'selected':''}} value="{{$type->id}}">[{{$type->code}}] {{$type->name}}</option>
                    @endforeach
            ...
            <div class="col-12 col-md form-group mb-0">
                ...
                @if(request()->query())
                <a href="{{route('cpanel.medizine.category.list')}}" class="text-xs text-danger ml-3">Reset Filter</a>
                @endif
            </div>
        </div>
    </form>
