    ...
    <form method="GET">
        <div class="row">
            ...
            <div class="col-12 col-md-2 form-group mb-0">
                <select name="sort_by" class="form-control form-control-sm form-select">
            ...
            <div class="col-12 col-md-3 form-group mb-0">
                <select name="program" class="form-control form-select select2" data-placeholder="Select Program">
                    <option value="" disabled>Select Program</option>
                    @foreach ($programs as $program)
                    <option {{request('program') == $program->id ? 'selected':''}} value="{{$program->id}}">[{{$program->code}}] {{$program->name}}</option>
                    @endforeach
                    <option {{!request('program') || request('program') == 'all' ? 'selected':''}} value="all">All Program</option>
                </select>
            </div>
            <div class="col-12 col-md-3 form-group mb-0">
            ...
            <div class="col-12 col-md form-group mb-0">
                ...
                @if(request()->query())
                <a href="{{route('cpanel.program.type.list')}}" class="text-xs text-danger ml-3">Reset Filter</a>
                @endif
            </div>
        </div>
    </form>
